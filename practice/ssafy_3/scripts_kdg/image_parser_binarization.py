...
class IMGParser:
    ...
    def callback(self, msg):
        ...
        lower_wlane = np.array([0, 0, 231])
        upper_wlane = np.array([255, 50, 245])

        lower_ylane = np.array([20, 120, 80])
        upper_ylane = np.array([45, 200, 255])
        ...
        img_wlane = cv2.bitwise_and(img_hsv, img_hsv, mask=img_wlane)
        img_wlane = cv2.cvtColor(img_wlane, cv2.COLOR_HSV2BGR)
        img_wlane = cv2.cvtColor(img_wlane, cv2.COLOR_BGR2GRAY)

        img_ylane = cv2.inRange(img_hsv, lower_ylane, upper_ylane)
        img_ylane = cv2.bitwise_and(img_hsv, img_hsv, mask=img_ylane)
        img_ylane = cv2.cvtColor(img_ylane, cv2.COLOR_HSV2BGR)
        img_ylane = cv2.cvtColor(img_ylane, cv2.COLOR_BGR2GRAY)
