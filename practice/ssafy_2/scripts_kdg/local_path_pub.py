...
# from scripts.global_path_pub import global_path_pub
    ...
    def __init__(self):
        ...
        while not rospy.is_shutdown():
            if self.is_odom == True and self.is_path == True:
                ...
                min_dis = float('inf')
                current_waypoint = -1
                for pose_idx in range(len(self.global_path_msg.poses)):
                    nx, ny = self.global_path_msg.poses[pose_idx].pose.position.x, self.global_path_msg.poses[pose_idx].pose.position.y
                    distance = sqrt((nx - x) ** 2 + (ny - y) ** 2)
                    if distance < min_dis:
                        min_dis = distance
                        current_waypoint = pose_idx
                # print(self.global_path_msg.poses[current_waypoint].pose.position)
                # print(self)
                ...
                if current_waypoint != -1 :
                    if current_waypoint + self.local_path_size < len(self.global_path_msg.poses):
                        for i in range(current_waypoint, current_waypoint + self.local_path_size):
                            ...
                            read_pose.pose.position.x = float(self.global_path_msg.poses[i].pose.position.x)
                            read_pose.pose.position.y = float(self.global_path_msg.poses[i].pose.position.y)
                            local_path_msg.poses.append(read_pose)
                    else :
                        for j in range(current_waypoint, len(self.global_path_msg.poses)):
                            read_pose = PoseStamped()
                            read_pose.pose.orientation.w = 1
                            read_pose.pose.position.x = float(self.global_path_msg.poses[j].pose.position.x)
                            read_pose.pose.position.y = float(self.global_path_msg.poses[j].pose.position.y)
                            local_path_msg.poses.append(read_pose)
                        for k in range(current_waypoint + self.local_path_size - len(self.global_path_msg.poses) + 1):
                            read_pose = PoseStamped()
                            read_pose.pose.orientation.w = 1
                            read_pose.pose.position.x = float(self.global_path_msg.poses[k].pose.position.x)
                            read_pose.pose.position.y = float(self.global_path_msg.poses[k].pose.position.y)
                            local_path_msg.poses.append(read_pose)
