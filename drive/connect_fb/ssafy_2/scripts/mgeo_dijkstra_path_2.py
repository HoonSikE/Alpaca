...
class dijkstra_path_pub :
    ...
    def init_callback(self,msg):
        ...
        shortest_node_idx = None
        distance = float('inf')
        for idx, node in self.nodes.items():
            node_x = node.point[0]
            node_y = node.point[1]

            temp = ((node_x - car_x)**2 + (node_y - car_y)**2)**0.5
            if temp < distance:
                shortest_node_idx = idx
                distance = temp

        self.start_node = shortest_node_idx
    ...
    def goal_callback(self,msg):
        ...
        for idx, node in self.nodes.items():
            ...
            temp = ((node_x - end_x)**2 + (node_y - end_y)**2)**0.5
            if temp < distance:
                shortest_node_idx = idx
                distance = temp

        self.end_node = shortest_node_idx
