...
class pure_pursuit :
    def __init__(self):
        ...
        while not rospy.is_shutdown():
            ...
            if self.is_path == True and self.is_odom == True and self.is_status == True:
                ...
                # 자동차와 로컬경로의 각도따라 속도가 감소되도록 설정
                # 앞의 변수는 자동차로부터 경로의 한 점을 잇는 직선과 지역경로의 선의 각도 차이
                # 뒤의 변수는 자동차의 yaw값과 지역경로의 선의 각도 차이
                # 두 변수 중 하나라도 특정 수치를 넘으면 도로를 크게 벗어났거나 벗어날 것이다.
                angle_diff, yaw_diff = self.path_angle_diff()
                # print(angle_diff, yaw_diff)

                print("before vel: {0}".format(self.target_velocity))

                # 도로에서 크게 벗어났거나 도로와 특정 각도 이상으로 벗어난 경우
                if angle_diff >= 50 or yaw_diff >= 20 :
                    # 20~30 사이의 값으로
                    temp_vel = max(20, min(30, (-1/3)*yaw_diff + 35))
                    # print(temp_vel)

                    # pid의 최대 속도를 설정해버린다.
                    self.target_velocity = min(self.target_velocity, temp_vel)
                
                print("after vel: {0}".format(self.target_velocity))
                # 여기까지 추가부분
    ...
    def path_angle_diff(self, ):
        # print(self.current_postion.x, self.current_postion.y)
        # print(self.vehicle_yaw  * (180 / pi))

        # print(self.status_msg.position.x, self.status_msg.position.y)
        # print(self.path.poses[0].pose.position.x, self.path.poses[0].pose.position.y)
        # print(self.path.poses[1].pose.position.x, self.path.poses[1].pose.position.y)

        # 자동차부터 로컬경로의 두번째 점까지의 각도
        first_local_deg = atan2(self.path.poses[1].pose.position.y - self.current_postion.y, self.path.poses[1].pose.position.x - self.current_postion.x) * (180 / pi)
        # 로컬경로의 두번째 점과 세번째 점까지의 각도
        second_local_deg = atan2(self.path.poses[2].pose.position.y - self.path.poses[1].pose.position.y, self.path.poses[2].pose.position.x - self.path.poses[1].pose.position.x) * (180 / pi)
...
class pidControl:
    def __init__(self):
        ...
        self.prev_error = 0
        self.i_control = 0
        self.controlTime = 0.03 # 왜 0.02 였지..? 30hz는 0.03 아닌가..?
