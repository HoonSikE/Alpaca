...
class pure_pursuit :
    def __init__(self):
        ...
        rospy.Subscriber("/global_path", global_path_callback) # 경로의 곡률을 이용한 속도 계획
        rospy.Subscriber("local_path", path_callback)
        rospy.Subscriber("odom", odom_callback)
        rospy.Subscriber("/Ego_topic", get_current_waypoint) # car speed
        ctrl_cmd_pub = rospy.Publisher('/ctrl_cmd', CtrlCmd, queue_size=2)
