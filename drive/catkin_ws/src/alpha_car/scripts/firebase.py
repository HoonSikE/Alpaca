...
class Basic_firebase:
    ...
    def dijkstra_weight_table(self):
        ...
        # 초기화
        for _, from_node in self.nodes.items():
            weight_from_this_node = dict()

            # init : possible next node, cost
    ...
    def dijkstra(self, startNode, endNode):
        ...
        while queue:
            ...
            for next_node in self.weight_table[start_idx]:
                ...
                if distance + contents[1] < distances[next_node]:
                    distances[next_node] = distance + contents[1]
                    shortest_ways[next_node] = shortest_ways[start_idx] + [contents[0]]
        ...
        for link_id in shortest_ways[endNode]:
            link_idx = link_id[0]
            changeable = link_id[1]
            if changeable == -1:
                link = self.links[link_idx]
                for point in link.points:
                    point_path.append([point[0], point[1], -1])
