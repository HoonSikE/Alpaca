...
class dijkstra_path_pub :
    ...
    def init_callback(self,msg):
        ...
        # PoseWithCovarianceStamped 형식의 ROS 메세지를 받아서 msg.pose.position 속성에서 x,y,z를 찾을 수 있다.
        min_dis = float('inf')
        for idx, val in self.nodes.items():
            x, y = val.point[0], val.point[1]
            distance = sqrt((x - msg.pose.pose.position.x) ** 2 + (y - msg.pose.pose.position.y) ** 2)
            if distance < min_dis:
                min_dis = distance
                self.start_node = idx
    ...
    def goal_callback(self,msg):
        ...
        # PoseStamped 형식의 ROS 메세지를 받아서 msg.pose.position 속성에서 x,y,z를 찾을 수 있다.
        min_dis = float('inf')
        for idx, val in self.nodes.items():
            x, y = val.point[0], val.point[1]
            distance = sqrt((x - msg.pose.position.x) ** 2 + (y - msg.pose.position.y) ** 2)
            if distance < min_dis:
                min_dis = distance
                self.end_node = idx
